...
EVENTS_PATH = Path(
    "results/research/opening_momentum/usdjpy/ny_event_surface_enriched_usdjpy.csv"
)

OUTPUT_DIR = Path(
    "results/research/opening_momentum/usdjpy/ny_v2_validation_gate"
)
...
# Para JPY pairs, 1 pip = 0.01 en precio.
PIP_SIZE_PRICE = 0.01
...
def build_base_trades(df: pd.DataFrame) -> pd.DataFrame:
    ...
    entry_price_col = f"entry_price_{ENTRY_DELAY}m"
    ...
    longs["entry_price"] = longs[entry_price_col]

    shorts["side"] = "short"
    shorts["gross_ret"] = -shorts[ret_col]
    shorts["entry_price"] = shorts[entry_price_col]
...
def add_costs(trades: pd.DataFrame, cost_pips: float) -> pd.DataFrame:
    ...
    # Convertir el coste en pips a retorno relativo:
    # cost_ret = (cost_pips * pip_size_en_precio) / entry_price
    out["cost_ret"] = (cost_pips * PIP_SIZE_PRICE) / out["entry_price"]
...
def main() -> None:
    ...
    print("\n=== USDJPY NY V2 VALIDATION GATE — GLOBAL ===\n")
    print(global_table.to_string(index=False))

    print("\n=== USDJPY NY V2 VALIDATION GATE — BY PERIOD ===\n")
    print(period_table.to_string(index=False))

    print("\n=== USDJPY NY V2 VALIDATION GATE — YEARLY (cost=1.0 pip) ===\n")
